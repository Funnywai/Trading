...
export async function scanMarket(
  onProgress: (_: string) => Promise<void>
): Promise<ScanResult> {
  const t0 = Date.now()
  ...
  await onProgress("📡 並行抓取 30 檔股票報價與新聞...")
  ...
  const results = await Promise.all(
    SCAN_UNIVERSE.map(async (ticker): Promise<StockSnapshot | null> => {
      try {
        ...
        completed++
        if (completed % 10 === 0) {
          await onProgress(`📡 正在抓取資料...（${completed}/${SCAN_UNIVERSE.length}）`)
        }
        return {
          ticker,
  ...
  if (valid.length === 0) {
    throw new Error("未找到任何有效標的，請稍後再試。")
  }

  await onProgress(`📊 已抓取 ${valid.length} 檔有效標的，LLM 分析中...`)
...
請輸出 ${SCAN_TOP_N} 檔評分最高的股票，按分數降冪排序。`
  ...
  try {
    ...
    throw new Error("LLM 回應格式錯誤，請稍後再試。")
  }

  if (parsed.length === 0) {
    throw new Error("LLM 未回傳任何有效結果。")
  }

  const priceMap = new Map(valid.map((s) => [s.ticker, s]))
  const scoredResults: ScoredResult[] = parsed.slice(0, SCAN_DISPLAY_N).map((r) => {
  ...
  const embeds = buildSearchEmbeds(scoredResults, valid.length, llmResult.tokensUsed, Date.now() - t0)
